  ...
  return {
    ...
    configureServer(server) {
      server.middlewares.use('/api/render-reel', async (req, res) => {
        ...
        req.on('end', async () => {
            const tempDir = path.resolve(process.cwd(), '.temp_render_' + Date.now() + '_' + Math.random().toString(36).substring(2, 7));
            try {
              const { broll, durations, frames, title, audioTrack } = JSON.parse(body);
              if (!durations || !frames || frames.length === 0) {
                res.statusCode = 400;
              ...
              // 1. Resolve B-roll path with fallback to 1.mp4
              let cleanBroll = (broll || '').trim();
              if (cleanBroll.startsWith('http://') || cleanBroll.startsWith('https://')) {
                try {
                  cleanBroll = new URL(cleanBroll).pathname;
                } catch(e) {}
              }
              if (cleanBroll.startsWith('/')) cleanBroll = cleanBroll.substring(1);

              let brollPath = path.resolve(process.cwd(), 'public', cleanBroll);
              if (!cleanBroll || !fs.existsSync(brollPath)) {
                console.warn(`[FFmpeg Server] B-roll '${cleanBroll}' not found, falling back to public/broll/1.mp4`);
                brollPath = path.resolve(process.cwd(), 'public', 'broll', '1.mp4');
              }
              ...
              if (audioTrack && audioTrack !== 'none') {
                let cleanAudio = audioTrack.trim();
                if (cleanAudio.startsWith('http://') || cleanAudio.startsWith('https://')) {
                  try {
                    cleanAudio = new URL(cleanAudio).pathname;
                  } catch(e) {}
                }
                if (cleanAudio.startsWith('/')) cleanAudio = cleanAudio.substring(1);
              ...
              const cleanOutputTitle = (title || 'reel').replace(/[\/\\:*?"<>|]/g, '_');
              const headerFilename = cleanOutputTitle.endsWith('.mp4') ? cleanOutputTitle : `${cleanOutputTitle}.mp4`;
              res.writeHead(200, {
                'Content-Type': 'video/mp4',
                'Content-Length': stat.size,
                'Content-Disposition': `attachment; filename="${encodeURIComponent(headerFilename)}"`,
